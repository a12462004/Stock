...
router.get('/', function(req, res, next) {
	...
	if(code == undefined){ //若為undefined代表沒有輸入code，直接跳轉頁面到Schedule的1101
		...
		new sql.ConnectionPool(db).connect().then(pool => {
  			return pool.request().query("SELECT Schedule.code,convert(varchar, event_date, 111) AS event_date,event_type,event_note,convert(varchar, [time], 108) AS event_time,place,content,StockPrice.clos,StockPrice.[open],StockPrice.up,StockPrice.down,StockPrice.trading_volume,StockPrice.price_date AS updateDate,CompanyProfile.capital,IndustryType.industry_name FROM Schedule INNER JOIN (SELECT TOP 1 * FROM StockPrice WHERE StockPrice.code='1101' ORDER BY price_date DESC) AS StockPrice ON Schedule.code = StockPrice.code INNER JOIN (SELECT CompanyProfile.code,industry,capital FROM CompanyProfile WHERE CompanyProfile.code='1101') AS CompanyProfile ON StockPrice.code=CompanyProfile.code  INNER JOIN (SELECT * FROM IndustryType) AS IndustryType ON CompanyProfile.industry = IndustryType.industry_id WHERE Schedule.code='1101' ORDER BY event_date DESC")
  		}).then(result => {
