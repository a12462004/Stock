...
router.post('/getChartData',function(req,res,next){
	...
	// console.log(code);
	new sql.ConnectionPool(db).connect().then(pool => {
  			return pool.request().query("SELECT YEAR([shareholders_date])-1 as year,round(cash_dividend,2) as cash_dividend,round((surplus+plot),2) as stock_dividend FROM [DividendPolicy] WHERE code='"+code+"'")
  		}).then(result => {
    		let rows = result.recordset
   			 res.setHeader('Access-Control-Allow-Origin', '*')
   		 	// res.status(200).json(rows);
