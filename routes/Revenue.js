...
"("+
  "select code,CONVERT(varchar(7),[r_montly],111) as 'date', Replace(CONVERT(NVARCHAR(20),CAST(Revenue.revenue AS money),1),'.00','') 'revenue',[m_growth_rate],[a_growth_rate],"+
  " Replace(CONVERT(NVARCHAR(20),CAST(Revenue.acc_revenue AS money),1),'.00','') 'acc_revenue',"+
  " [acc_growth_rate],"+
  " CAST([last_revenue] as INT) 'last_revenue' from [dbo].[Revenue]"+
  " where code = '"+code+"'"+
"), [StockPrice] as "+ 
"("+
" select top 1  code,[company],CAST ( YEAR(StockPrice.price_date) AS VARCHAR(10) )+'/'+CAST ( MONTH(StockPrice.price_date) AS VARCHAR(10) )+'/'+CAST ( DAY(StockPrice.price_date) AS VARCHAR(10) ) as 'updateDate',[open],[up],[down],[clos],Replace(CONVERT(NVARCHAR(20),CAST(StockPrice.trading_volume AS money),1),'.00','') AS trading_volume from "+
" [dbo].[StockPrice]"+
" where code ='"+code+"'"+
" order by price_date desc "+
"),[companyprofile] as "+
"("+
" select code, Replace(CONVERT(NVARCHAR(20),CEILING(CAST(CompanyProfile.capital AS money)/1000000),1),'.00','') AS capital,[IndustryType].industry_name from [dbo].[CompanyProfile] join[dbo].[IndustryType]"+
...
router.post('/getChartData',function(req,res,next){
	...
	// console.log(code);
	new sql.ConnectionPool(db).connect().then(pool => {
        return pool.request().query("select convert(varchar(7), a.[r_montly], 111) as 'r_montly' , a.[a_growth_rate], cast(a.[revenue] as int) as 'revenue', b.clos from [dbo].[Revenue] a join [dbo].[StockPrice]  b on a.code = b.code and a.r_montly = b.price_date where a.code ='"+code+"' order by a.[r_montly] ")
